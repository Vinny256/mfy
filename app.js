...
async function initApp() {
    ...
    if (yearSelect) {
        for (let y = 2026; y >= 2005; y--) {
            yearSelect.innerHTML += `<option class="bg-zinc-900" value="${y}">${y}</option>`;
        }
    }
    fetchGenres();
    fetchTrendingHero();
...
// 2. MISSING FUNCTIONS (The fix for your console errors)
...
async function fetchGenres() {
    try {
        ...
        if (genreList) {
            genreList.innerHTML = data.genres.map(g => `
                <button onclick="filterByGenre(${g.id})" class="text-left text-[10px] hover:text-red-500 transition uppercase font-bold tracking-tighter">${g.name}</button>
            `).join('');
        }
...
async function fetchTrendingHero() {
    ...
    document.getElementById('heroRating').innerHTML = `<i class="fas fa-star mr-1"></i> ${item.vote_average ? item.vote_average.toFixed(1) : 'N/A'}`;
    document.getElementById('heroYear').innerText = (item.release_date || item.first_air_date || '').split('-')[0];
    
    document.getElementById('heroPlay').onclick = () => openDetails(item.id, item.media_type || 'movie');
    document.getElementById('heroMore').onclick = () => openDetails(item.id, item.media_type || 'movie');
}
    ...
    grid.innerHTML = items.map(item => `
        <div class="relative group cursor-pointer overflow-hidden rounded-lg bg-zinc-900" onclick="openDetails('${item.id}', '${type}')">
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
                <h3 class="font-bold text-[10px] truncate">${item.title || item.name}</h3>
            </div>
            <div class="absolute top-2 right-2 bg-black/70 px-1.5 py-0.5 rounded text-[9px] font-black text-yellow-500">
                <i class="fas fa-star scale-75"></i> ${item.vote_average ? item.vote_average.toFixed(1) : '0.0'}
            </div>
        </div>
    `).join('');
}
...
function showGathuoNotice() {
    ...
    notice.innerHTML = `
        <div class="bg-zinc-900 border border-white/10 p-8 rounded-3xl max-w-md text-center shadow-2xl">
            ...
            <h2 class="text-2xl font-black mb-4 tracking-tighter">GATHUO VALUES YOU</h2>
            <p class="text-zinc-400 text-sm leading-relaxed mb-8">Ads may show. Just return to enjoy your movie. If it fails, change server.</p>
            <button onclick="document.getElementById('gathuoNotice').remove(); prepareToWatch();" class="bg-white text-black w-full py-4 rounded-xl font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition">Let's Play</button>
        </div>`;
...
document.getElementById('searchInput').addEventListener('keyup', async (e) => {
    ...
    if (query.length > 2) {
        ...
        const data = await res.json();
        displayGrid(data.results.filter(i => i.poster_path), 'latestGrid', 'movie');
    }
});

// Category Helpers & Player
async function fetchLatestReleases() {
    const res = await fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${API_KEY}`);
    const data = await res.json();
    displayGrid(data.results, 'latestGrid', 'movie');
}
...
async function loadEpisodes(seasonNum) {
    ...
    document.getElementById('episodeSelect').innerHTML = data.episodes.map(e => `<option class="bg-zinc-900" value="${e.episode_number}">Ep ${e.episode_number}: ${e.name}</option>`).join('');
}
